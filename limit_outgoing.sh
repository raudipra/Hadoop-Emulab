...
slowDevice(){
	...
	sudo tc qdisc del dev $1 root > /dev/null 2>&1
	echo "============================================================"
	echo "Slowing down interface $1 to $4"
	echo "============================================================"
	# add handling classes
	sudo tc qdisc add dev $1 root handle 1:0 htb default 2
	sudo tc class add dev $1 parent 1:0 classid 1:1 htb rate $2 ceil $2
	sudo tc class add dev $1 parent 1:1 classid 1:2 htb rate $3 ceil $3
	sudo tc class add dev $1 parent 1:1 classid 1:3 htb rate $4 ceil $4
	sudo tc qdisc add dev $1 parent 1:2 sfq
	sudo tc qdisc add dev $1 parent 1:3 sfq
	# filter by ip
	sudo tc filter add dev $1 parent 1:0 protocol ip u32 match ip dst $5/32 flowid 1:3
        sudo tc filter add dev $1 parent 1:0 protocol ip u32 match ip src $5/32 flowid 1:3
    # done
}
# ===============================================================================================================
# MAIN
...
do
	slowDevice ${REMOTE_DEVICE_PRELUDE}${deviceId} $MAX_TOTAL_RATE $MAX_SUBCLASS_RATE $SLOW_RATE $TARGET
done
